...
public abstract class MGESingleInputRecipeCache <INPUT, INGREDIENT extends InputIngredient<INPUT>, RECIPE extends MekanismRecipe<?> & Predicate<INPUT>,
        CACHE extends IInputCache<INPUT, INGREDIENT, RECIPE>> extends MGEAbstractInputRecipeCache<RECIPE> {
    private final Set<RECIPE> complexRecipes = new HashSet<>();
    private final Function<RECIPE, INGREDIENT> inputExtractor;
    private final CACHE cache;

    protected MGESingleInputRecipeCache(MGERecipeType<?, RECIPE, ?> recipeType, Function<RECIPE, INGREDIENT> inputExtractor, CACHE cache) {
        super(recipeType);
     ...
     */
    @Nullable
    public <DATA_1, DATA_2> RECIPE findTypeBasedRecipe(@Nullable Level world, INPUT input, DATA_1 data1, DATA_2 data2, SingleInputRecipeCache.CheckRecipeType<INPUT, RECIPE, DATA_1, DATA_2> matchCriteria) {
        if (cache.isEmpty(input)) {
            //Don't allow empty inputs
            return null;
        }
