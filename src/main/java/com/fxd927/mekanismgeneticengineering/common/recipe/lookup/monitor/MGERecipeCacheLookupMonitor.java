package com.fxd927.mekanismgeneticengineering.common.recipe.lookup.monitor;
...
public class MGERecipeCacheLookupMonitor <RECIPE extends MekanismRecipe<?>> implements ICachedRecipeHolder<RECIPE>, IContentsListener {
    private final IMGERecipeLookupHandler<RECIPE> handler;
    ...
    public MGERecipeCacheLookupMonitor(IMGERecipeLookupHandler<RECIPE> handler) {
        this(handler, 0);
    }

    public MGERecipeCacheLookupMonitor(IMGERecipeLookupHandler<RECIPE> handler, int cacheIndex) {
    ...
    public void loadSavedData(@NotNull CachedRecipe<RECIPE> cached, int cacheIndex) {
        if (cachedIndexMatches(cacheIndex)) {
            ...
            if (cached instanceof ItemStackConstantChemicalToObjectCachedRecipe<?, ?> c &&
                    handler instanceof IMGERecipeLookupHandler.ConstantUsageRecipeLookupHandler lookupHandler) {
