package com.fxd927.mekanismgeneticengineering.client.gui.element.progress;

import com.fxd927.mekanismgeneticengineering.client.recipe_viewer.interfaces.IMGERecipeViewerRecipeArea;
...
public class MGEGuiProgress  extends GuiTexturedElement implements IMGERecipeViewerRecipeArea<MGEGuiProgress>, ISupportsWarning<MGEGuiProgress> {
    ...
    @Nullable
    private MGEGuiProgress.ColorDetails colorDetails;
    @Nullable
    private BooleanSupplier warningSupplier;

    public MGEGuiProgress(IProgressInfoHandler.IBooleanProgressInfoHandler handler, ProgressType type, IGuiWrapper gui, int x, int y) {
        this((IProgressInfoHandler) handler, type, gui, x, y);
    }

    public MGEGuiProgress(IProgressInfoHandler handler, ProgressType type, IGuiWrapper gui, int x, int y) {
        super(type.getTexture(), gui, x, y, type.getWidth(), type.getHeight());
        this.type = type;
        this.handler = handler;
    }

    public MGEGuiProgress colored(MGEGuiProgress.ColorDetails colorDetails) {
        this.colorDetails = colorDetails;
        return this;
    }

    @Override
    public MGEGuiProgress warning(@NotNull WarningTracker.WarningType type, @NotNull BooleanSupplier warningSupplier) {
    ...
    @NotNull
    @Override
    public MGEGuiProgress recipeViewerCategories(@NotNull IRecipeViewerRecipeType<?>... recipeCategories) {
