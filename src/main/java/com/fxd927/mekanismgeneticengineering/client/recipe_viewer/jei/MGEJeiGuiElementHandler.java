...
public class MGEJeiGuiElementHandler implements IGuiContainerHandler<GuiMekanism<?>> {
    private final IIngredientManager ingredientManager;

    public MGEJeiGuiElementHandler(IIngredientManager ingredientManager) {
    ...
    private Collection<IGuiClickableArea> getGuiClickableArea(List<? extends GuiEventListener> children, double mouseX, double mouseY) {
        for (GuiEventListener child : children) {
            if (child instanceof ContainerEventHandler eventHandler) {
                ...
                if (child instanceof IMGERecipeViewerRecipeArea<?> recipeArea && recipeArea.isRecipeViewerAreaActive() && child instanceof GuiElement element && element.visible) {
                    ...
                    if (categories != null && recipeArea.isMouseOverRecipeViewerArea(mouseX, mouseY)) {
                        ...
                        IGuiClickableArea clickableArea = IGuiClickableArea.createBasic(element.getRelativeX(), element.getRelativeY(),
                                element.getWidth(), element.getHeight(), MGEJEI.recipeType(categories));
